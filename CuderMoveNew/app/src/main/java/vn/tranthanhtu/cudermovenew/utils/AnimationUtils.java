...
public final class AnimationUtils {
    ...
    /* Play animation when listMove added */
    ...
    private static void createAnimatorList(
            ArrayList<String> listMove,
            ImageView imageView,
            MapAdapter adapter,
            Context context,
            String edt)
    {
        for (int count = 0; count < listMove.size(); count++) {
            MapModel.list
                    .set(ConvertLocationUtils.convertToLocation(x, y) - 1
                            , new MapModel(Constants.WAY_WILL_GO));
            ...
            if (MapModel
                    .list
                    .get(ConvertLocationUtils.convertToLocation(x, y) - 1)
                    .getSquare()
